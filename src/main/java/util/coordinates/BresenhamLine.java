...
public class BresenhamLine {
    ...
    public static void doPerLineCoord(int x1, int y1, int z1, final int x2, final int y2, final int z2, Consumer<Vec3i> consumer) {
        ...
        if ((l >= m) && (l >= n)) {
            ...
            err_2 = dz2 - l;
            for (i = 0; i < l; i++) {
                consumer.accept(new Vec3i(point[0], point[1], point[2]));
                if (err_1 > 0) {
                    point[1] += y_inc;
                    err_1 -= dx2;
                }
...
void doPerLineCoord(int x1, int y1, int z1, const int x2, const int y2, const int z2, WorldMap *output, int symbol){
